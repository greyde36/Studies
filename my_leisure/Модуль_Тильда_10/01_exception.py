...
# все это - исключительные ситуации, exceptions. Это объекты класса с таким названием.
...
# То есть в момент нештатной ситуации интерпретатор создает объект нужного класса и "бросает" его (raise)
# Куда бросает? в стек вызовов - чуть позже разберем что это такое.
# Если программа поймала этот объект исключительной ситуации - то она будет устойчивой к ошибкам,
# и это хорошо. Если не поймала - то выполнение останавливается.

# Место, где возникло исключение, сохраняется в аттрибуте __traceback__ объекта исключения