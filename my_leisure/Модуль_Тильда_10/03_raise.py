...
def greet_person(person_name):
    ...
    if person_name == 'Robert':
        ...
        # тогда автоматически создастся объект исключения без параметров
        raise BaseException
    print(f'Hi there {person_name}')
...
# и передачи ответственности вовне - пусть там решают что делать дальше
...
# автоматом прицепляется объект породившего исключения, в атрибут __cause__
...
# Иногда возникает необходимость переопределить содержимое объекта исключения - это можно сделать вот так
