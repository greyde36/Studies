...
# Можно работать как с обычными объектами
...
# Конечно же функции, _принимающие_ параметры, тоже могут быть переданы через переменную/параметр
# (помните, что переменные суть ярлыки-ссылки на объекты?)
def adder(*args):
    res = 0
    for number in args:
        res += number
...
# map и filter принимают любые объекты, по которым можно пройти циклом:
