...
result = digits(3141, 5926, 2718, 2818)
print(result)
# и теперь digits - почти та же функция, но не та. Она от декодирована функцией time_track
...
# Минусы декораторов:
#  - затруднена отладка
#  - нужно делать определенные действия, что бы сохранить аттрибуты декодируемой функции (см functools.wraps)